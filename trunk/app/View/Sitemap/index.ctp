...
<ul><li class="parent_sitemap">Events</li>
    ...
    <?php } ?>
    <p /></ul>
<ul><li class="parent_sitemap">Product<p /></li>
    <ul><li class="parent_sitemap">Media</li>
        <?php for ($i = 0; $i < count($menu4); $i++) { ?>
            <ul><li class="parent_sitemap">
                    <?php echo $this->Html->link(__('Products'), array('controller' => 'Products', 'action' => 'product_details')); ?><p />
                </li></ul>
        <?php } ?>
        <p />
